...
function CountryFlagValidator({ password }: CountryFlagValidatorProps) {
    ...
    return (
        <div className="section">
            <p className="section-title">Validátor vlajky</p>

            {}
            <img
                src={`https://flagcdn.com/w80/${selectedCountry.toLowerCase()}.png`}
                alt={`Vlajka ${selectedCountry}`}
                style={{ width: 100, borderRadius: 3, marginBottom: "0.5rem" }}
            />

            <p className={untouched ? "text-muted" : isValid ? "text-success" : "text-error"}>
                {untouched
                    ? `Heslo musí obsahovat zkratku země`
                    : isValid
